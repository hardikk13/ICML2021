...
float SphericalNode(float x, float y, float z, const Point& p0, float r) {
  ...
  const float diff_y = y - p0.y;
  const float diff_z = z - p0.z;
...
float StrutLattice(float x, float y, float z, float* strut_pts, std::uint64_t num_edges,
  ...
  for (std::uint64_t i = 1; i < num_edges; ++i) {
    ...
    const float f0_sphere = SphericalNode(x, y, z, p0, r * 2.f);
    const float f1_sphere = SphericalNode(x, y, z, p1, r * 2.f);
